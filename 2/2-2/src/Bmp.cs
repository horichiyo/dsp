	...
	public static class Bmp
	{
		...
		public static RgbData[,] ToConvertBitmap(byte[] target, BITMAPINFOHEADER bih)
		{
			...
			for(int i = 0; i < bih.biWidth; i++)
			{
				...
				{
					result[i, j].Blue = target[i * bih.biBitCount / 8 + j * Stride];
					result[i, j].Green = target[i * bih.biBitCount / 8 + j * Stride + 1];
					result[i, j].Red = target[i * bih.biBitCount / 8 + j * Stride  + 2];
				}
			}
			return result;
		}
		...
		public static byte[] ToConvertArray(RgbData[,] target, BITMAPINFOHEADER bih)
		{
			...
			for(int i = 0; i < bih.biWidth; i++)
			{
				...
				{
					result[i * bih.biBitCount / 8 + j * Stride] = target[i, j].Blue;
					result[i * bih.biBitCount / 8 + j * Stride + 1] = target[i, j + 1].Green;
					result[i * bih.biBitCount / 8 + j * Stride + 2] = target[i, j + 2].Red;
				}
			}
			return result;
		}
